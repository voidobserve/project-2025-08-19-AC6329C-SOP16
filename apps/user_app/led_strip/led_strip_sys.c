...
{
    if(fc_effect.star_speed_index > 0) fc_effect.star_speed_index--;
    one_wire_set_period(motor_period[fc_effect.star_speed_index ]);
    enable_one_wire();
...
{
    if(fc_effect.star_speed_index < 4)fc_effect.star_speed_index++;
    one_wire_set_period(motor_period[fc_effect.star_speed_index ]);
    enable_one_wire();
   printf("fc_effect.star_speed_index = %d",fc_effect.star_speed_index);
...
void ls_set_motor_off(void)
{
 ...
 mo_cnt = 8;
    // one_wire_set_mode(6);  //停止电机
    // enable_one_wire();
    // os_time_dly(4);   使用这个函数，在森木处有风险。不建议使用
    // enable_one_wire();
    fb_motor_switch();
}
